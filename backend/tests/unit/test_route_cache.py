...
def test_make_cache_key_uses_version_and_node_ids_only():
    assert make_cache_key("v1", "opts", "a", "b") == "v1|opts|a|b"
    assert make_cache_key("v1", "opts", "a", "b") == make_cache_key("v1", "opts", "a", "b")
...
def test_get_moves_entry_to_mru_for_lru_eviction():
    ...
    cache.put("v", "o", "a", "b", CachedGraphPath(["a", "b"], 1.0))
    cache.put("v", "o", "c", "d", CachedGraphPath(["c", "d"], 2.0))
    assert cache.get("v", "o", "a", "b") is not None
    cache.put("v", "o", "e", "f", CachedGraphPath(["e", "f"], 3.0))
    assert cache.get("v", "o", "c", "d") is None
    assert cache.get("v", "o", "a", "b") is not None
    assert cache.size == 2
...
def test_lru_evicts_oldest_when_exceeding_limit():
    ...
    assert cache.get("v", "o", "n0", "m0") is None
    assert cache.get("v", "o", "n1000", "m1000") is not None


def test_reverse_lookup_does_not_synthesize_from_forward_entry():
    cache = RouteCache()
    cache.put(
        "hcm-fixture-v2",
        "opts",
    ...
    assert cache.get("hcm-fixture-v2", "opts", "node-end", "node-start") is None
    assert cache.size == 1


def test_different_graph_version_is_distinct_key():
    cache = RouteCache()
    cache.put("v1", "o", "a", "b", CachedGraphPath(["a", "b"], 1.0))
    assert cache.get("v2", "o", "a", "b") is None
    assert cache.size == 1
...
def test_service_reverse_computes_separate_cache_entry_on_directed_graph(runtime):
    ...
    assert reverse.from_cache is False
    assert runtime.route_cache.size == size_one + 1
...
def test_oneway_reverse_is_not_served_from_forward_cache(tmp_path):
    ...
    forward = find_cached_or_compute_graph_path(runtime, (10.5, 106.5), (10.51, 106.51))
    assert forward.from_cache is False
    with pytest.raises(NoRouteError):
        find_cached_or_compute_graph_path(runtime, (10.51, 106.51), (10.5, 106.5))
    assert runtime.route_cache.get(
        runtime.metadata.graph_version,
        "[]|[]",
        "b",
        "a",
    ) is None
